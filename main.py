...
#
# class xVideo:
#     def __init__(self, device, model, nic):
#         nicArr = []
#         for item in nic:
#             nicArr.append(NIC(item["Description"], item["MAC"], item["Timestamp"], item["Rx"], item["Tx"]))
#
#         self.nic = None
#         self.device = device
#         self.model = model
...
#
#     def printNIC(self):
#         for item in self.nic:
#             item.printInfo()
#
#     def getNIC(self):
#         nicString = "["
#         for item in self.nic:
#             nicString += item.toString()
#             if len(self.nic) > 1:
#                 nicString += ", "
#         nicString += "]"
#         return nicString
#
#     def printInfo(self):
#         print("Device: {transDevice} Model: {transModel} NIC: {transNic}".format(
#             transDevice=str(self.device), transModel=str(self.model), transNic=str(self.getNIC())))
#
#
# class NIC:
#     def __init__(self, description, macAddress, timeStamp, rx, tx):
...
#
#     def printInfo(self):
#         print("Description: {nicDescription} MAC: {nicMac} Timestamp: {nicTimestamp} Rx: {nicRx} Tx: {nicTx}".format(
#             nicDescription=str(self.description), nicMac=str(self.macAddress), nicTimestamp=str(self.timeStamp),
#             nicRx=str(self.rx), nicTx=str(self.tx)))
#
#     def toString(self):
#         return "Description: {nicDescription} MAC: {nicMac} Timestamp: {nicTimestamp} Rx: {nicRx} Tx: {nicTx}".format(
#             nicDescription=str(self.description), nicMac=str(self.macAddress), nicTimestamp=str(self.timeStamp),
#             nicRx=str(self.rx), nicTx=str(self.tx))
#
#     def getBitRate(self):
#         self.rxBitRate = (int(self.rx) / 2 + 10)/10000000
#         print(self.rxBitRate)
